...
// Ported from netlify/functions/panel-stats.js.
export async function GET() {
  ...
  const [configs, states] = await Promise.all([repo.getConfigs(), repo.listStates()]);

  const byPost: Record<
    string,
    { id: string; label: string; dmSent: number; dmFailed: number; clickedGetLink: number; becameFollower: number; linkClicked: number }
  > = {};
  ...
  for (const s of states) {
    const bucket = s.configId ? byPost[s.configId] : undefined;
  ...
  return NextResponse.json({ totals, byPost: Object.values(byPost), rows: rows.slice(0, 10) });
}
